...
export const submitOrder = async (req, res) => {
  try {
    ...
    // إنشاء الطلب الجديد مباشرة
    const orderData = { ...req.body, order_number: generateOrderNumber() };
    const newOrder = await Order.create(orderData);

    // ✅ تعيين أقرب سائق
    const closestDriver = await assignClosestDriver(newOrder.customer.coords);
    if (closestDriver) {
      newOrder.assigned_staff_id = closestDriver._id;
      newOrder.status = "in_transit";
      await newOrder.save();
      ...
      // إشعار السائق
      const io = req.app.get("io");
      const driverSocketId = getActiveDriversMap().get(closestDriver._id.toString());
      if (io && driverSocketId) {
        io.to(driverSocketId).emit("new-order-assigned", {
          order_number: newOrder.order_number,
          customer: newOrder.customer,
